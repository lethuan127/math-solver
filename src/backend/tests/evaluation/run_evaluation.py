...
class EvaluationRunner:
    ...
    def _calculate_summary_metrics(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for result in detailed_results:
            ...
            if expected == actual:
                exact_matches += 1
            elif expected in actual or actual in expected or self._is_numerically_equivalent(expected, actual):
                partial_matches += 1
            else:
                no_matches += 1
    ...
    async def _save_csv_summary(self, results: Dict[str, Any], csv_file: Path) -> None:
        ...
        with open(csv_file, 'w', newline='') as f:
            ...
            # Header
            writer.writerow([
                "Test Case", "Image File", "Expected Answer", "Actual Answer", 
                "Exact Match", "Partial Match", "Answer Length", "Expected Length"
            ])
            ...
            for i, result in enumerate(detailed_results, 1):
                ...
                partial_match = (expected.lower() in actual.lower() or 
                               actual.lower() in expected.lower() or
                               self._is_numerically_equivalent(expected, actual))
    ...
    def _print_evaluation_report(self, results: Dict[str, Any]) -> None:
        ...
        print(f"🎯 Exact Accuracy: {summary['exact_accuracy']:.1f}% ({summary['exact_matches']}/{summary['total_cases']})")
        print(f"🎯 Partial Accuracy: {summary['partial_accuracy']:.1f}% ({summary['exact_matches'] + summary['partial_matches']}/{summary['total_cases']})")
        ...
        print(f"\n📏 RESPONSE ANALYSIS:")
        print(f"   Average Answer Length: {summary['average_answer_length']:.1f} characters")
        print(f"   Average Expected Length: {summary['average_expected_length']:.1f} characters")
        ...
        for i, result in enumerate(results["detailed_results"], 1):
            ...
            if expected.strip().lower() == actual.strip().lower():
                status = "✅ EXACT"
            elif (expected.strip().lower() in actual.strip().lower() or 
                  actual.strip().lower() in expected.strip().lower() or
                  self._is_numerically_equivalent(expected, actual)):
                status = "⚡ PARTIAL"
            else:
                status = "❌ MISS"
...
async def main():
    ...
    parser.add_argument("--no-report", action="store_true", help="Don't print detailed report")
