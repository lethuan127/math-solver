...
class MathSolverEvaluator:
    ...
    def __init__(self, config_path: str = None):
        # Load config from deepeval_config.py in root
        import os
        import sys
        ...
        self.math_solver = OpenAIMathSolverService()
        self.test_cases = []
    ...
    async def solve_math_problem(self, image_path: str) -> str:
        ...
        try:
            ...
            response = await self.math_solver.solve_problem(upload_file)

            # Extract the answer value from the MathAnswer response
            if isinstance(response, MathAnswer):
                return response.answer_value
            elif hasattr(response, "answer_value"):
                return response.answer_value
            else:
                return str(response)
    ...
    def create_llm_test_cases(
        self, test_cases: list[dict[str, Any]], actual_outputs: list[str]
    ) -> list[LLMTestCase]:
        ...
        for i, (case, actual_output) in enumerate(zip(test_cases, actual_outputs)):
            test_case = LLMTestCase(
                ...
                context=[
                    f"Mathematical problem solving from image {case['image_file']}"
                ],
            )
    ...
    async def run_evaluation(self, max_cases: int = None) -> dict[str, Any]:
        ...
        return {
            ...
            "detailed_results": [
                ...
                for case, output, llm_case in zip(
                    test_cases, actual_outputs, llm_test_cases
                )
            ],
        }
...
@pytest.mark.parametrize(
    "case_index", range(30)
)  # Adjust range based on your test cases
async def test_individual_math_problem(case_index):
    ...
    if case_index >= len(test_cases):
        pytest.skip(
            f"Test case {case_index} not available (only {len(test_cases)} cases)"
        )
...
async def test_full_math_solver_evaluation():
    ...
    for i, detail in enumerate(results["detailed_results"], 1):
        status = (
            "✓"
            if detail["expected"].strip().lower() in detail["actual"].strip().lower()
            else "✗"
        )
